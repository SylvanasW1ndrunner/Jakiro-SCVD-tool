...
def test(test_dataloader, cnnmodel,vunl):
    checkpoint = torch.load(cnnmodel)
    cnnmodel = CNN_Model(768).to(device)
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            _, cls_embeddings = bertembedding(batch['text'], model, tokenizer)
            cls_embeddings = torch.tensor(cls_embeddings, dtype=torch.float32).to(device)
            output = cnnmodel(cls_embeddings)
...
def train(train_dataloader, test_dataloader,vunl):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < best_loss:
            counter = 0
            best_loss = avg_loss
            torch.save(cnnmodel.state_dict(), vunl +'_' +'sc_' +'best_cnnmodel.pth')
        else:
            counter += 1
            if counter >= patience:
    ...
    ##test
    test(test_dataloader, vunl+'_'+'sc_'+'best_cnnmodel.pth',vunl)
